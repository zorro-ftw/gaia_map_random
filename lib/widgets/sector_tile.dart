...
class SectorTile {
  ...
  SectorTile({this.sectorNumber, this.rotation, this.mapConfig});

  List<Planet> getPlanetList() {
    List<Planet> sector = [];

    //Checking if it's hollowside and sector number is 5,6 or 7
    if(mapConfig == MapConfig.HollowSide && sectorNumber == 5 || sectorNumber == 6 || sectorNumber == 7){
      switch(sectorNumber) {
        case 5:
          {
